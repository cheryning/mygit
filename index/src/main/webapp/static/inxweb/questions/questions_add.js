...
function addQuestions(){
	...
	if(title.trim()==""){
		$("input[name='questions.title']").parent().next().html('<img width="16" height="16" alt="错误" src="'+baselocation+'/static/inxweb/img/c-icon.png">');
		dialog('提示信息',"问答标题不能为空",1);
		return;
	}else if(title.length<4){
		$("input[name='questions.title']").parent().next().html('<img width="16" height="16" alt="错误" src="'+baselocation+'/static/inxweb/img/c-icon.png">');
		dialog('提示信息',"问答标题长度不能小于4个字",1);
		return;
	}else{
		$("input[name='questions.title']").parent().next().html('<img width="16" height="16" alt="正确" src="'+baselocation+'/static/inxweb/img/d-icon.png">');
	}
	
	var questionsContent=$("textarea[name='questions.content']").val();
	if(questionsContent.trim()==""){
		$("textarea[name='questions.content']").next().html('<img width="16" height="16" alt="错误" src="'+baselocation+'/static/inxweb/img/c-icon.png">');
		dialog('提示信息',"问答内容不能为空",1);
		return;
	}else{
		$("textarea[name='questions.content']").next().html('<img width="16" height="16" alt="正确" src="'+baselocation+'/static/inxweb/img/d-icon.png">');
	}
	
	var questionsTag=$("#questionsTag").val().trim();
	if(questionsTag==""){
		$("#tags-content").next().html('<img width="16" height="16" alt="错误" src="'+baselocation+'/static/inxweb/img/c-icon.png">');
		dialog('提示信息',"请至少选择一个标签",1);
		return;
	}else{
		$("#tags-content").next().html('<img width="16" height="16" alt="正确" src="'+baselocation+'/static/inxweb/img/d-icon.png">');
	}
	
	var randomCode=$("input[name='randomCode']").val();
	if(randomCode.trim()==""){
		$("input[name='randomCode']").parent().next().next().html('<img width="16" height="16" alt="错误" src="'+baselocation+'/static/inxweb/img/c-icon.png">');
		dialog('提示信息',"请输入验证码",1);
		return;
	}else{
		$("input[name='randomCode']").parent().next().next().html('<img width="16" height="16" alt="正确" src="'+baselocation+'/static/inxweb/img/d-icon.png">');
	}
	...
	$.ajax({
		...
		success:function(result){
			if(result.success==true){
				window.location.href="/index/questions/info/"+result.entity;
			}else{
...
function checkTitleLength(obj){
	...
	if(title.length<8){
		$(obj).parent().next().html('<img width="16" height="16" alt="错误" src="'+baselocation+'/static/inxweb/img/c-icon.png">');
	}else{
		$(obj).parent().next().html('<img width="16" height="16" alt="正确" src="'+baselocation+'/static/inxweb/img/d-icon.png">');
	}
}
