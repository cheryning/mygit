...
function getPlayerHtml(kpointId,name,obj) {
	...
	$.ajax({
		url : "" + baselocation + "/index/front/ajax/getKopintHtml",
		data : {
			"kpointId" : kpointId,
			"courseId" : otherId
		},
...
function favorites(courseId,obj) {
	$.ajax({
		url : baselocation + '/index/front/createfavorites/' + courseId,
		type : 'post',
		dataType : 'json',
		success : function(result) {
 ...
 function addNotest() {
 	...
 	if (KindEditor.get("#notesContextId").isEmpty()) {
 		...
 		$.ajax({
 			url : baselocation + "/index/courseNote/ajax/addnote",
 			type : 'post',
 			dataType : 'json',
 			data : {
...
function queryNote(obj) {
	...
	$(obj).addClass("current");
	$.ajax({
		url : baselocation + "/index/courseNote/ajax/querynote",
		type : "post",
		dataType : "text",
		data : {
			'kpointId' : currentKpointId,
...
function addPlayTimes(courseId, kpointId) {
	$.ajax({
		url : baselocation + "/index/couserStudyHistory/ajax/playertimes",
		data : {
			"kpointId" : kpointId,
			"courseId" : courseId
		},
