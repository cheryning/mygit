								...
								<div class="cc-l-wrap">
									...
									<c:if test="${course.currentPrice=='0.00' }">
										...
										<span class="fl jgAttr c-ccc f-fA">
											<tt class="c-999 f-fA"><c:if test="${course.currentPrice=='0.00' }">${course.pageBuycount }</c:if><c:if test="${course.currentPrice!='0.00' }">${course.pageBuycount }</c:if>人学习</tt>
											|
											<tt class="c-999 f-fA"><c:if test="${course.currentPrice=='0.00' }">${course.pageViewcount }</c:if><c:if test="${course.currentPrice!='0.00' }">${course.pageViewcount }</c:if>浏览</tt>
										</span>
									</section>
								</div>
							</li>
